    ...
    if request.user.is_authenticated:
        ...
        for item in Post.objects.filter(owner__in=followerslist).order_by('-date_uploaded'):
            ...
            for like in Like.objects.filter(postid=item.id, liker=request.user.username):
                mylike = 1
            out.append({"PostID": item.id, "URL": item.image, "Content": item.content, "Owner": item.owner, "DateUploaded": item.date_uploaded.strftime("%Y-%m-%d %H:%M:%S"), "ProfilePic": profilepics[item.owner], "Comment":commentlist, "Likes": item.likes, "Mylike":mylike})
        # 내 지역 이벤트 찾기 10개만
        fcnt = 0
...
def notice(request):
    ...
    if request.user.is_authenticated:
        ...
        for item in Notice.objects.all().order_by('-date_uploaded'): #공지사항 가져오기
            cnt = cnt+1
            out.append({"NoticeID": item.id, "Content": item.content, "Owner": item.owner, "DateUploaded": item.date_uploaded.strftime("%Y-%m-%d %H:%M:%S"), "Title": item.title})
